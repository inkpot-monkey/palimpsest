  ...
  nodes = {
    server =
      { lib, ... }:
      {
        ...
        # Stub the gate option the profile reads; left at its default (off) so the persist
        # block is inert here — reboot-persistence is the standard ADR-0004 mechanism, verified
        # by the rk1b config eval, not re-proven in this sandbox.
