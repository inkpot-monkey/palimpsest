...
The bespoke email bridge ([ADR-0007](0007-custom-jmap-matrix-bridge.md)) was developed in-tree under `pkgs/jmap-matrix-bridge/`, with its NixOS service module and VM round-trip check alongside it. It now lives in a standalone (private) repository, `palebluebytes/jmap-matrix-bridge`, consumed here as a flake input over `git+ssh` (the same scheme as the `secrets` input, since the repo is private): the input's overlay supplies `pkgs.jmap-matrix-bridge` fleet-wide, and its `nixosModules.jmap-bridge` is imported by the matrix profile. Only the host-specific glue — sops templates, the tuwunel/identity wiring in `modules/nixos/profiles/matrix/jmap-bridge.nix` — stays in this repo.
...
- The repo owner is `palebluebytes` (matching the crate's long-standing `meta.homepage`), distinct from this repo's `inkpot-monkey` GitHub account. It is private for now, so the input is `git+ssh` and evaluation needs SSH access to GitHub (already required by the `secrets` input); making it public later would let the input switch to the lighter `github:` fetcher.
