...
(use-package
 ...
 (defun unpop-to-mark-command+ ()
   ...
   (when mark-ring
     ...
     (set-marker (mark-marker) (car (last mark-ring))
                 (current-buffer))
     (when (null (mark t))
       (ding))
...
With a prefix ARG, save it to the kill ring instead of inserting it."
   (interactive "P")
   (let ((token
          (substring (shell-command-to-string "openssl rand -hex 32")
                     0 -1)))
     (if arg
         (progn
           (kill-new token)
...
(use-package
 ...
 :ensure nil
 :bind
 (:map prog-mode-map ("C-c l" . display-line-numbers-mode)))
...
(use-package
 ...
 :config
 (dolist (var '("NIX_PATH" "NIX_SSL_CERT_FILE"))
...
(use-package
 ...
 :bind
 (:map embark-collect-mode-map ("i" . wgrep-change-to-wgrep-mode))
 (:map
  wgrep-mode-map
...
(use-package sops :init (global-sops-mode t))
...
(use-package
 ...
 :config (auth-source-sops-enable))
...
(use-package
 ...
 (ef-themes-headings '((0 extrabold 1.5) (t variable-pitch 1.0)))
...
(use-package
 adaptive-wrap
 :ensure t
 :config (setq-default adaptive-wrap-extra-indent 0))
...
(use-package
 ...
 :bind (:map wdired-mode-map ("s-q" . wdired-exit)))
...
(use-package
 ...
 :after dired
 :bind
 (:map dired-mode-map ("C-c C-t" . whisperx-transcribe-dired)))
...
(use-package link-hint :bind ("M-J" . link-hint-open-link))
...
(use-package
 vertico
 :init (vertico-mode)
 :custom (vertico-cycle t) (vertico-resize t)
...
(use-package just-ts-mode :mode ("\\Justfile\\'" . just-ts-mode))

(use-package just-complete :bind (("C-c j" . just-run+)))
...
(use-package
 ...
 :after corfu
 :config (corfu-candidate-overlay-mode t))
...
(use-package
 ...
 (defun my/completion-at-point ()
   (cape-wrap-super #'cape-abbrev #'cape-dabbrev #'cape-keyword))
...
(use-package
 ghostel
 :custom (ghostel-module-auto-install nil)
...
(use-package
 ...
 :bind (("C-, g" . ai-code-menu))
 :config (ai-code-set-backend 'gemini)
...
(use-package journalctl-mode :bind (("C-c t" . journalctl)))
...
(use-package eww :ensure nil :bind ("s-w" . eww))

(use-package
 recall
 :bind ("C-x C-r" . recall-rerun)
 :custom
 (recall-save-file
 ...
 (recall-mode t) (run-with-idle-timer 60 t (lambda () (recall-save))))
...
(use-package
 ...
 :bind
 (:map markdown-mode-map ("C-c V" . markdown-view-mode))
 (:map markdown-view-mode-map ("C-c V" . markdown-mode))
 (:map markdown-mode-map ("C-c v" . gfm-view-mode))
 (:map gfm-view-mode-map ("C-c v" . gfm-mode))
...
(use-package
 ...
 (defun my/eglot-capf ()
   (add-hook 'completion-at-point-functions
             (cape-capf-super
              #'eglot-completion-at-point #'cape-abbrev)
             nil t))
...
(use-package
 ement
 :commands
 (ement-connect ement-list-rooms ement-view-room ement-describe-room)
 :custom
 (ement-save-sessions t)
 (ement-auto-sync t)
 (ement-initial-sync-timeout 40))

(use-package ement-glue :after ement :bind ("C-c m" . ement-glue-map))
...
(use-package
 ...
 :commands
 (nix-rebuild-system+
  nix-update-system-flake+ nix-system-network-transient))

(use-package pretty-sha-path :init (global-pretty-sha-path-mode))
...
(use-package
 ...
 :init (push '(html-mode . html-ts-mode) major-mode-remap-alist)
 :bind
 (:map
  html-ts-mode-map
  ("M-o" . nil)
...
(use-package
 ...
 :init (push '(css-mode . css-ts-mode) major-mode-remap-alist)
...
buffer (*App: Name*)."
   (let* ((cmd (get-text-property 0 :exec cand))
          ...
          (clean-cmd
           (and cmd
                (replace-regexp-in-string " %[a-zA-Z].*" "" cmd))))
     (if clean-cmd
         (let ((buffer-name
                (format "*App: %s*"
           ...
           (start-process-shell-command
            "nixos-app" (generate-new-buffer buffer-name) clean-cmd))
       (message "Error: No executable found for %s"
                (get-text-property 0 :title cand)))))

 :bind ("C-c s" . consult-omni-launch))

(use-package consult-omni-launch :commands (consult-omni-launch))
