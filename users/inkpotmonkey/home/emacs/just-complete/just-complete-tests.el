...
(ert-deftest test-just-complete--parse-args ()
  ...
  (should
   (equal (just-complete--parse-args "host") '(("host" nil nil))))
  (should
   (equal
    (just-complete--parse-args "target='release'")
    '(("target" "release" nil))))
  (should
   (equal (just-complete--parse-args "*args") '(("args" nil t))))
  (should
   (equal
    (just-complete--parse-args "a b c")
    '(("a" nil nil) ("b" nil nil) ("c" nil nil))))
  (should
   (equal
    (just-complete--parse-args "a='b' c='d'")
    '(("a" "b" nil) ("c" "d" nil))))
  (should
   (equal
    (just-complete--parse-args "a=\"b\" c=d")
...
(ert-deftest test-just-complete--get-candidates-mocked ()
  ...
  (let* ((temp-dir (make-temp-file "just-complete-test-" t))
    ...
    (with-temp-file mock-script
      (insert "#!/bin/sh\n")
      (insert
       "echo '    deploy host                     # Deploy to host'\n")
      (insert
       "echo '    build target='\\''release'\\''          # Build target'\n")
      (insert
    ...
    (let ((default-directory temp-dir))
      (let ((candidates (just-complete--get-candidates)))
        ...
        (let ((cand1 (nth 0 candidates)))
          ...
          (should
           (string= (get-text-property 0 'just-args cand1) "host"))
          (should
           (string=
            (get-text-property 0 'just-comment cand1)
            "Deploy to host")))
        ...
        (let ((cand2 (nth 1 candidates)))
          ...
          (should
           (string=
            (get-text-property 0 'just-args cand2)
            "target='release'"))
          (should
           (string=
            (get-text-property 0 'just-comment cand2)
            "Build target")))
        ...
        (let ((cand3 (nth 2 candidates)))
          ...
          (should
           (string= (get-text-property 0 'just-args cand3) "*args"))
          (should
           (string=
            (get-text-property 0 'just-comment cand3) "Run tests")))))
  ...
  (let* ((temp-dir (make-temp-file "just-complete-test-" t))
    ...
    (let ((default-directory temp-dir))
      (let ((candidates (just-complete--get-candidates)))
        ...
        (should
         (string=
          (get-text-property 0 'just-args (nth 0 candidates)) ""))
        (should
         (string=
          (get-text-property 0 'just-comment (nth 0 candidates)) ""))
...
(ert-deftest test-just-complete--marginalia-registration ()
  ...
  (let ((marginalia-annotator-registry '((test-category))))
    (cl-letf (((symbol-function 'boundp)
               (lambda (sym)
                 (eq sym 'marginalia-annotator-registry))))
      (if (boundp 'marginalia-annotators)
          (error "Should not be here")
