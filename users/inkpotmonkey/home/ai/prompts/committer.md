...
1. If nothing is staged, review `git status` and `git log --oneline -5` for context
1. Analyze the changes and group them into logical, independent commits
...
1. Plan the commit grouping aloud
1. Use `git add <file>` or `git add -p` to stage changes
1. Commit each group with `git commit -m "<message>"`
1. Repeat for each logical chunk until all changes are committed
