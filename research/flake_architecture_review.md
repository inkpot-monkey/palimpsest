...
1. **Feature-Based Organization**: Profiles (`modules/nixos/profiles`) group related settings (gaming, desktop, ai) rather than scattering them across hosts.
1. **Encapsulation**: User code stays in `users/`, system code in `hosts/`, and shared logic in `modules/`.
