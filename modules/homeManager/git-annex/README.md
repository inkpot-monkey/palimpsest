...
1. **Bot Key** (`id_annex_autostart`): **Passwordless**. Used *only* by the background `git-annex assistant`.
...
1. **Authorize the Key (Server Side)**:
...
1. **Authorize**: Add the public key to `authorized_keys` on the server.
1. **Result**: Both interactive logins and the background assistant will work immediately without extra configuration.
...
1. **Challenge**: Systemd user services do not automatically inherit environment variables (like `SSH_AUTH_SOCK`) from your login shell.
1. **Solution**: You must ensure `SSH_AUTH_SOCK` is imported into the systemd user environment.
...
1. **Get UUID**: Run `git annex info` on the gateway to get its UUID.
1. **Update Client Config**: Add the UUID to `expectedUUID` in your Home Manager config.
1. **Deploy Client**: `home-manager switch ...` (or `nixos-rebuild` if using HM module in NixOS).
