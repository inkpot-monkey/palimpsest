...
in
{
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf cfg.enable {
      xdg.configFile = 
        let
          mkInitService = name: repo:
            let
              script = pkgs.writeShellScript "git-annex-init-${name}" ''
                  set -e
                  export PATH="${lib.makeBinPath [ pkgs.git pkgs.git-annex pkgs.gnupg pkgs.openssh pkgs.rsync pkgs.coreutils pkgs.gnugrep pkgs.gawk ]}:$PATH"
          ...
          assistantUnit = pkgs.writeText "git-annex-assistant.service" ''
            ...
            Wants=${lib.concatMapStringsSep " " (n: "git-annex-init-${n}.service") (lib.attrNames cfg.repositories)}
            After=network.target ${lib.concatMapStringsSep " " (n: "git-annex-init-${n}.service") (lib.attrNames cfg.repositories)}

            [Service]
            ExecStart=${pkgs.git-annex}/bin/git-annex assistant --autostart
            Type=forking
            Restart=on-failure
            Environment="PATH=${lib.makeBinPath [ pkgs.git pkgs.git-annex pkgs.gnupg pkgs.openssh pkgs.rsync ]}:/usr/bin:/bin"

            [Install]
            WantedBy=default.target
          '';

          assistantFiles = if cfg.assistant.enable then {
          ...
          autostartFile = 
            let
              enabledRepos = lib.filterAttrs (n: v: v.assistant) cfg.repositories;
              repoPaths = lib.mapAttrsToList (n: v: v.path) enabledRepos;
              allPaths = cfg.assistant.autostartPaths ++ repoPaths;
            in
            if allPaths != [] then {
              "git-annex/autostart".text = lib.concatStringsSep "\n" allPaths;
              "git-annex/autostart".onChange = "${pkgs.systemd}/bin/systemctl --user restart git-annex-assistant.service || true";
            } else {};
            
          initFiles = lib.foldl' (acc: name: acc // (mkInitService name cfg.repositories.${name})) {} (lib.attrNames cfg.repositories);
        in
