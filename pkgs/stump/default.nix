...
# consumer keeps saying `pkgs.stump`. nixpkgs' own package claims x86_64-linux
# only, so on the way out rk1b (aarch64) needs the `meta.platforms` widening this
# file already carries — as a `modifications` overlay entry; that shape is in git
# history at the commit that deleted this file.
#
# Why carry it: 0.1.5 CANNOT serve the KOReader progress round-trip, and that
# round-trip is palimpsest#116. `GET /koreader/{key}/syncs/progress/{document}`
# builds its query with `reading_session::Entity::find()` and then maps the rows
# into `ModelWithDevice`, whose `FromQueryResult` reads *prefixed* columns — so
# the moment a reading session actually exists the mapping fails and the route
# 500s. Progress can be pushed and never read back. Upstream stumpapp/stump#1279;
# the fix is the one-line `ModelWithDevice::find()` in #1280, released in 0.1.6.
# Standing the catalog up at 0.1.6 also means it is created fresh and never runs
# the 0.1.5 reading-session migration, which shipped with a data-loss warning.
#
# THIS FILE HAS BEEN DELETED ONCE AND BROUGHT BACK (see `git log --grep 'in-repo
# 0.1.6 override'`). The deletion was right about the cost and wrong about the
# need: 0.1.6 is in no binary cache, so every consumer compiles a Rust workspace
# and a yarn frontend — the `supernote_mirror` check took over forty minutes to
# reach its first VM. That cost is now knowingly paid, because the alternative is
# shipping a sync integration whose fetch half is broken by construction. Budget
# for it, or build on the fleet's remote builder; rk1b pays nothing extra, since
# nixpkgs declares stump x86_64-only and it compiles Stump either way.
#
# The exit is close: nixpkgs took 0.1.6 on 2026-08-08 (commit 10fc77b5c), so the
# first `nix flake update nixpkgs` that crosses that date retires this file AND
# restores a cached x86_64 build. Check the removal condition then.
#
