...
class SmartLLMHook:
    ...
    def __init__(
        self, model: str = "ollama/qwen2.5:7b", api_base: str = "http://localhost:11434"
    ):
        """
        Initialize the hook.
    ...
    def __call__(self, entries, existing_entries):
        ...
        if not self._check_ollama():
            print(
                f"Warning: Ollama not reachable at {self.api_base}. Skipping AI tagging.",
                file=sys.stderr,
            )
            return entries
        ...
        for entry in existing_entries:
            if isinstance(entry, data.Transaction):
                ...
                for p in entry.postings:
                    if p.units.number > 0 and any(
                        p.account.startswith(pat) for pat in self.expense_patterns
                    ):
                        expense_posting = p
                        break
            ...
            if isinstance(entry, data.Transaction):
                ...
                if needs_help:
                    ...
                    if description in history_cache:
                        ...
                        print(
                            f"Ref: Found cached entry for '{description}' -> {cached['account']}",
                            file=sys.stderr,
                        )
                        predicted = {
                    ...
                    if predicted:
                        ...
                        print(
                            f"DEBUG: Applying prediction {predicted}", file=sys.stderr
                        )
                        ...
                        expense_posting = data.Posting(
                            predicted["account"], units, None, None, None, None
                        )
                        ...
                        print(
                            f"DEBUG: Prediction failed/empty for {description}",
                            file=sys.stderr,
                        )
    ...
    def verify_integrity(self, original_entries, processed_entries):
        ...
        if len(original_entries) != len(processed_entries):
            raise RuntimeError(
                f"Integrity Failure: Entry count mismatch. Original: {len(original_entries)}, Processed: {len(processed_entries)}"
            )
        ...
        for i, (orig, proc) in enumerate(zip(original_entries, processed_entries)):
            ...
            if orig.date != proc.date:
                raise RuntimeError(
                    f"Integrity Failure at index {i}: Date changed from {orig.date} to {proc.date}. Transaction: {orig.narration}"
                )

            # 2. Anchor Check (Only for Transactions)
            if isinstance(orig, data.Transaction) and isinstance(
                proc, data.Transaction
            ):
                ...
                if not proc.postings:
                    raise RuntimeError(
                        f"Integrity Failure at index {i}: Processed transaction has no postings. Original had {len(orig.postings)}."
                    )
                ...
                if anchor_orig.account != anchor_proc.account:
                    raise RuntimeError(
                        f"Integrity Failure at index {i}: Anchor account changed. Original: {anchor_orig.account}, Processed: {anchor_proc.account}"
                    )
        ...
        try:
            response = completion(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are a helpful accounting assistant. Return JSON only.",
                    },
